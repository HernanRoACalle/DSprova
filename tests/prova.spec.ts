...
test('submissão completa da prova', async ({ browser }) => {
  ...
  //Questão 6
  await page.getByLabel("A porcentagem do código de produção executada pela suite de testes").click();
  await page.getByRole('button', { name: 'Próxima' }).click();

  //Questão 7
  await page.getByLabel("Incluir na lista de dependências um valor que é reatribuído dentro do próprio efeito").click();
  await page.getByRole('button', { name: 'Próxima' }).click();

  //Questão 8
  await page.getByLabel("Um conjunto de casos de teste agrupados em torno de uma funcionalidade ou preocupação").click();
  await page.getByRole('button', { name: 'Próxima' }).click();
  
  //Questão 9
  await page.getByLabel("Uma função que valida os dados de entrada, salva no banco de dados e envia um e-mail").click();
  await page.getByRole('button', { name: 'Próxima' }).click();
  
  //Questão 10
  await page.getByLabel("Para manter o teste rápido e determinístico, sem depender de um serviço externo").click();
  await page.getByRole('button', { name: 'Próxima' }).click();

  //Questão 11
  await page.getByLabel("Testa um único trecho de lógica de forma isolada, sem dependências externas").click();
  await page.getByRole('button', { name: 'Próxima' }).click();
  
  //Questão 12
  await page.getByLabel("Uma tupla com o valor atual do estado e uma função de atualização").click();
  await page.getByRole('button', { name: 'Próxima' }).click();
  
  //Questão 13
  await page.getByLabel("Executa uma função de configuração antes de cada caso de teste individualmente").click();
  await page.getByRole('button', { name: 'Próxima' }).click();
  
  //Questão 14
  await page.getByLabel("Os testes devem ser simples e legíveis — evite complexidade desnecessária").click();
  await page.getByRole('button', { name: 'Próxima' }).click();
  
  //Questão 15
  await page.getByLabel("Don't Repeat Yourself — evite duplicar conhecimento ou lógica").click();
  await page.getByRole('button', { name: 'Próxima' }).click();
  
  //Questão 16
  await page.getByLabel("Quantas vezes a função foi chamada e com quais argumentos").click();
